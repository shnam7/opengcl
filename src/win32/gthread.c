...
extern gthread_key_t	_gkey_self;		/* this is referenced by gtime.c */
...
static inline int _check_init()
{
	...
	if ( _tcb_main.h_thread == 0 )
	{
		...
		if ( !DuplicateHandle(h_process, h_thread, h_process, &h_thread,
...
GCLAPI int gthread_cancel(gthread_t gt)
{
	__gthread_tcb *_tcb = (__gthread_tcb *)gt;
	if ( IsBadWritePtr(_tcb, sizeof(*_tcb)) )
		return ERROR_INVALID_HANDLE;
	_tcb->canceled = 1;
	SetEvent( _tcb->h_cancel );
	return 0;
}
...
GCLAPI int gthread_key_create(gthread_key_t *key, void (*destr_func)(void *) )
{
...
GCLAPI int gthread_attr_setdetachstate(gthread_attr_t *attr,
									   int _detachstate)
{
	if ( _detachstate != GTHREAD_CREATE_JOINABLE
			&& _detachstate != GTHREAD_CREATE_DETACHED )
	{
		return ERROR_INVALID_DATA;
	}
...
/* general pointer storage: primary used for GThread class */
/*
