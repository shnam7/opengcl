...
int gque_test()
{
	...
	printf( "*----- Full list -----*\n" );
	count = 0;
	unsigned entries = gq.getEntries();
	unsigned linear = gq.getLinearEntries();
	unsigned rolled = entries - linear;
...
int gque_variable_unsignedest()
{
	...
	for (; i<50; ++i)
	{
		...
		while ( !gq.requestLinearRooms(size) )
		{
			...
			unsigned dsize = vd->dataSize;
			gq.shiftHead( vd->dataSize );

			printf( "vd of size=%d was poped. head=%d, tail=%d, end=%d rem=%d\n",
				dsize, (unsigned)(gq.head()-gq.begin()), (unsigned)(gq.tail()-gq.begin()),
				(unsigned)(gq.end()-gq.begin()), gq.remainder() );
		}
		...
		printf( "vd of size=%d was added -- head=%d, tail=%d, end=%d rem=%d\n",
				size, (unsigned)(gq.head()-gq.begin()), (unsigned)(gq.tail()-gq.begin()),
				(unsigned)(gq.end()-gq.begin()), gq.remainder() );
        }
	return 0;
}
...
{
	//return gque_test();
	return gque_variable_unsignedest();
